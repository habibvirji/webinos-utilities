...
function copyTestPage(node_moduleDir, apiName, name) {
    try {
        ...
        if (apiName.indexOf("node_modules")) {
            var name = node_moduleDir.split("/");
            apiName = name[name.length-1];
        }
        ...
        if (stat.isDirectory()){
            ...
            for (k = 0 ; k < testFile.length; k = k + 1){
                stat = fs.statSync(path.join(node_moduleDir, apiName, "test", "web_root", name, testFile[k]));
                if (stat.isDirectory()){
...
exports.checkForWebinosModules = function(node_moduleDir) {
    try {
        ...
        for (i = 0; i < fileList.length; i = i + 1) {
            if (fileList[i]==="webinos-utilities") fileList[i] = path.join("webinos-utilities", "node_modules", "webinos-api-test");
...
exports.createWebinosJS = function(node_moduleDir, apiModules) {
    ...
    { 
        ...
        var wrtDir = config.android.wrt;
        if (!existsSync (wrtDir)) 
            fs.mkdirSync (wrtDir);
        staticPath = config.android.static;
